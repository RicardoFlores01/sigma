...
/* MOSTRAR VENTANA DE BUSQUEDA */ 
  ...
  if (urlActual.includes(busquedaNormal) || urlActual.includes(busquedaNormal_filent)) {
    const div = document.getElementById('busquedadiv');
    div.style.display = 'block';
  }

  if (buscarCortev !== undefined && urlActual.includes(buscarCortev)) {
    const buscarCorte = document.getElementById('busquedaCorte');
    buscarCorte.style.display = 'block';
  }

  if (dwn !== undefined && urlActual.includes(dwn)) {
...
/* MAPA */ 
  let coord_x;
  let coord_y;
  //let extent = [-13459673.0276619,1232095.46159631,-9281930.46183394,4109451.87806631];
...
/* MOSTRAR CAPAS BASE */ 
  ...
  mdm.addEventListener("change", comprobarCapasBase);
...
/* CODIGO PARA MOSTRAR LOS LAYERS */ 
...
/* FUNCIONES DEL ICONO DE ACCESO RAPIDO */ 
  ...
  // altitud puntero en forma de punto 
...
/* DEMAS FUNCIONES */
  ...
  // ocultar header 
    function ocultar_header(){
      ...
      document.getElementById('icon_access_bottom').style.display = 'none';
      document.getElementById('dropdown').style.borderBottom = '1px solid white';
      document.getElementById('menu_btn').style.borderBottom = '1px solid white';
    }
  ...
  // mostrar header 
    function mostrar_header(){
      ...
      document.getElementById('icon_access_bottom').style.display = 'block';
      document.getElementById('dropdown').style.borderBottom = '1px solid #BDBDBD';
      document.getElementById('menu_btn').style.borderBottom = '1px solid #BDBDBD';
    }
  ...
  function reload(){
    ...
    var corte = 2022; 
    function descargar(){
      ventana = window.open('descarga.jsp?corte='+corte,'Buscar','toolbar=no,Resizable=1,scrollbars=1');
      ventana.focus();
    }

  function yesreload(){
    window.location.href = "http://localhost:8070/mapas/mapa.jsp?#map=5.00/-11416123.69/2708933.29/0";
  }
...
/* AGREGAR PUNTO */
  ...
  let nombreElement = document.getElementById("nombre"); // Reemplaza "nombre" por el identificador único de tu elemento HTML
                        ...
                        fetch(url)
                            ...
                            .then(function (json) {
                                ...
                                if (data.features.length > 0) {
                                  let nombre = data.features[0].properties.nom_ent; // Suponiendo que el nombre se encuentra en la propiedad "nom_ent" del primer feature
                                  console.log(nombre);
